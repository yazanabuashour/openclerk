...
| New natural-intent revisit | `compile_synthesis` as one narrow create-or-update call | 34 | 8 | 65.51s | Natural user intent with outcome-level guidance | `none` in repaired targeted eval | Low observed risk: preserved existing candidate, `source_refs`, current/superseded source status, source paths, `## Sources`, and `## Freshness`. |
| New scripted-control revisit | None; exact current primitive workflow | 28 | 9 | 57.12s | Scripted-control | `none` in repaired targeted eval | Low observed risk: exact current primitives preserved authority, provenance, freshness, and duplicate-prevention checks. |
...
or observed bypass risk, while the uncoached negative-limit validation control
still failed final-answer-only behavior. Promotion should not follow from high
command counts alone. It should follow only if natural intent repeatedly
produces these failures while the scripted control proves the current
primitives remain technically sufficient.

## POC Conclusion

Do not promote `compile_synthesis` from the POC or repaired targeted evidence.
The repaired synthesis rows and remaining validation gap support the
defer/reference decision in
...
validation repair should remain separate from `compile_synthesis` promotion.
