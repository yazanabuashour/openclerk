# Capture Low-Risk Ceremony Eval

## Status

Implemented targeted eval lane for `oc-hhap`.
...
whether low-risk capture ceremony should promote a future surface, stay
reference evidence, defer, or be killed.

## Purpose

This eval pressure-tests routine low-risk capture where the user supplies
explicit note body content but omits `document.path` and `document.title`. The
safe current ceiling is propose-before-create: infer a candidate path, title,
and faithful body; validate the candidate; check runner-visible duplicate
evidence when needed; and ask for approval before any durable write.
...
mise exec -- go run ./scripts/agent-eval/ockp run \
  --parallel 1 \
  --scenario capture-low-risk-natural-intent,capture-low-risk-scripted-control,capture-low-risk-duplicate-check,missing-document-path-reject,negative-limit-reject,unsupported-lower-level-reject,unsupported-transport-reject \
  --report-name ockp-capture-low-risk-ceremony
```

## Scenario Families

- `capture-low-risk-natural-intent`: natural low-risk save request with
...
- `capture-low-risk-scripted-control`: exact validation control for the same
  candidate; no create is allowed before approval.
- `capture-low-risk-duplicate-check`: seeded runner-visible similar note;
