---
decision_id: adr-capture-low-risk-ceremony
decision_title: Capture Low-Risk Ceremony
decision_status: accepted
decision_scope: low-risk-capture
decision_owner: platform
decision_date: 2026-04-30
source_refs: docs/evals/capture-low-risk-ceremony.md, docs/evals/results/ockp-capture-low-risk-ceremony.md
---
# Capture Low-Risk Ceremony Promotion Decision
...
- [`docs/evals/capture-low-risk-ceremony.md`](../evals/capture-low-risk-ceremony.md)
- [`docs/evals/results/ockp-capture-low-risk-ceremony.md`](../evals/results/ockp-capture-low-risk-ceremony.md)

## Evidence

The targeted `capture-low-risk-ceremony` lane ran with `gpt-5.4-mini`,
reasoning effort `medium`, parallelism `1`, and release blocking `false`. The
reduced report recorded natural low-risk save intent, scripted candidate
validation, duplicate checks, no-bypass controls, tool/command count,
assistant calls, wall time, prompt specificity, UX, brittleness, retries, step
count, latency, guidance dependence, safety risks, and evidence posture fields.

Lane result: `promote_low_risk_capture_surface_design`.

Scenario summary:

| Scenario | Classification | Tools / commands | Assistant calls | Wall seconds | Safety risks |
| --- | --- | ---: | ---: | ---: | --- |
| `capture-low-risk-natural-intent` | `ergonomics_gap` | 6 / 6 | 3 | 23.55 | `none_observed` |
| `capture-low-risk-scripted-control` | `none` | 4 / 4 | 3 | 13.09 | `none_observed` |
| `capture-low-risk-duplicate-check` | `none` | 8 / 8 | 4 | 26.42 | `none_observed` |
| validation controls | `none` | 0 / 0 | 1 each | 5.13-10.58 | `none_observed` |
...
Request shape: a normal user asks OpenClerk to save routine low-risk note
content while omitting `document.path` and `document.title`.
...
Capability pass: passed for current primitives. The scripted validation and
duplicate-check controls completed with `none` classifications through
existing `openclerk document` and `openclerk retrieval` behavior. This does
not prove a runner capability gap.

UX quality: not acceptable enough to keep only as reference pressure. A normal
user would expect OpenClerk to turn explicit low-risk note content into a safe
propose-before-create candidate without exact prompt choreography. The natural
row failed with an `ergonomics_gap`, while the scripted validation and
duplicate controls required 4-8 command executions and 3-4 assistant calls.
