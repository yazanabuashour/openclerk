...
- Agent-facing task policy in `skills/openclerk/SKILL.md` and module `SKILL.md` files, including direct-reject rules, runner-only routine workflows, synthesis freshness, provenance inspection, and lower-level bypass handling.
...
- Skill policy changes should run `./scripts/validate-all-agent-skills.sh` and the relevant OpenClerk production or targeted agent eval gate before release.
