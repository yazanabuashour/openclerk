...
This AGENTS.md is the user's standing explicit instruction to use available orchestration tools when the main agent judges they materially reduce risk, improve independent coverage, or save enough wall-clock time to justify the overhead.

Do not wait for the task prompt to name subagents, threads, or worktrees.

Prefer:
- the main session for architecture, task decomposition, integration, final verification, review sequencing, and commits;
- scoped read-only subagents for broad audits, unfamiliar-codebase exploration, source/API verification, test-gap analysis, fixture ideas, security/correctness/complexity review, and other independent investigation;
- isolated worktrees for parallel implementations, dependency experiments, large migrations, or risky edits that would disturb the active checkout;
...
Do not use extra agents for tightly coupled edits, small one-file fixes, vague exploratory churn, or tasks where coordination overhead would dominate. Do not delegate away architectural ownership: parallel agents must report scope, files inspected or changed, commands run, result, risks, and next recommended action, and the main session integrates all work under the normal review and commit workflow.
