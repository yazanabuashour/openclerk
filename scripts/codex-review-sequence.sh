...
complexity_prompt='Review the current uncommitted changes. Focus on avoidable complexity: Rule of Three, YAGNI, the simplest correct implementation, and clearer library/API simplifications. Report only actionable simplifications with file:line references and why the simpler alternative preserves behavior. If there are none, say exactly: No actionable avoidable-complexity findings.'
