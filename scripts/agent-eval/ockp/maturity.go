...
func writeSyncDiagnosticsMarkdown(b *strings.Builder, label string, diagnostics *sqlite.SyncDiagnostics) {
	...
	fmt.Fprintf(b, "| fts_strategy | `%s` |\n", diagnostics.FTSStrategy)
	fmt.Fprintf(b, "| fts_bootstrap | %t |\n", diagnostics.FTSBootstrap)
	fmt.Fprintf(b, "| fts_rebuild_pending | %t |\n", diagnostics.FTSRebuildPending)
	fmt.Fprintf(b, "| fts_rebuild_skipped | %t |\n", diagnostics.FTSRebuildSkipped)
	...
	fmt.Fprintf(b, "| document_record_write_seconds | %.2f |\n", diagnostics.DocumentRecordWriteSeconds)
	fmt.Fprintf(b, "| chunk_write_seconds | %.2f |\n", diagnostics.ChunkWriteSeconds)
	fmt.Fprintf(b, "| provenance_write_seconds | %.2f |\n", diagnostics.ProvenanceWriteSeconds)
	fmt.Fprintf(b, "| incremental_fts_write_seconds | %.2f |\n", diagnostics.IncrementalFTSWriteSeconds)
	fmt.Fprintf(b, "| bulk_fts_rebuild_seconds | %.2f |\n", diagnostics.BulkFTSRebuildSeconds)
